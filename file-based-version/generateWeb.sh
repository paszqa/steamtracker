...
echo "img.imglarge{ border-radius: 10px; max-width: 100%; height: auto; -o-transition:.5s; -ms-transition:.5s; -moz-transition:.5s; -webkit-transition:.5s; transition:.5s; }" >>  "/var/www/html/steamtracker/$steamName-$steamId.php"
echo "img.imglarge:hover{ filter: brightness(1.4); -o-transition:.5s; -ms-transition:.5s; -moz-transition:.5s; -webkit-transition:.5s; transition:.5s; max-width: 100%; height: auto;}" >>  "/var/www/html/steamtracker/$steamName-$steamId.php"
...
echo "<th id='A' class='firstrow' onclick=\"if(this.id=='A'){ sortTable(1,'asc',0); this.id='B';}else{sortTable(1,'desc',0); this.id='A';}\" style='width: 200px;'>Game</th>">>  "/var/www/html/steamtracker/$steamName-$steamId.php"
dateArray=()
whichtable=0
for i in $(seq 0 $lastDays); do
	...
	if [ $i == $lastDays ]; then
		echo "<th id='A' class='dates' onclick=\"if(this.id=='A'){ sortTable($(($whichtable+1)),'asc',1); this.id='B';}else{sortTable($(($whichtable+2)),'desc',1); this.id='A';}\">PLAYED<br>RECENTLY</th>" >>  "/var/www/html/steamtracker/$steamName-$steamId.php"
		echo "<th id='A' class='dates' onclick=\"if(this.id=='A'){ sortTable($(($whichtable+2)),'asc',1); this.id='B';}else{sortTable($(($whichtable+3)),'desc',1); this.id='A';}\">PLAYED<br>TOTAL </th></tr>" >>  "/var/www/html/steamtracker/$steamName-$steamId.php"
	fi
